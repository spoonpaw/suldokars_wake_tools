<script lang="ts">
   ...
   *   - Weapons
   *   - Armor
   *   - Inventory (gear + vehicles + pets — same EquipmentItem shape)
   *
  ...
  let modalInitialTab = $state<'weapons' | 'armor' | 'gear' | 'vehicles' | 'pets' | 'custom'>('weapons');
  ...
  const encColor = $derived(
    enc === 'ok' ? 'text-neutral-400' : enc === 'overloaded' ? 'text-amber-300' : 'text-red-400'
  );
...
<div class="space-y-4">
            ...
            <li class="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div class="flex flex-wrap items-baseline justify-between gap-2">
                <span class="font-semibold text-neutral-100">
                  {w.name}
                  {#if !w.equipped}<span class="ml-2 text-xs text-neutral-500">(not equipped)</span>{/if}
                </span>
          ...
          {#each character.armor as a (a.id)}
            <li class="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div class="flex flex-wrap items-baseline justify-between gap-2">
                <span class="font-semibold text-neutral-100">
                  {a.name}
                  {#if a.isShield}<span class="ml-1 text-xs text-blue-300">[Shield]</span>{/if}
                  {#if a.isHelmet}<span class="ml-1 text-xs text-blue-300">[Helmet]</span>{/if}
                  {#if a.primeOnly}<span class="ml-1 text-xs text-amber-300">§</span>{/if}
                  {#if !a.equipped}<span class="ml-2 text-xs text-neutral-500">(not equipped)</span>{/if}
                </span>
  ...
  <!-- INVENTORY (gear + vehicles + pets) -->
  <Card title="Inventory">
    {#if character.inventory.length === 0}
      {#if mode === 'edit'}
        <EmptyState title="Empty" message="Pick gear, vehicles, or pets from the catalog, or add a custom item." />
      {:else}
        ...
        <ul class="space-y-2">
          {#each character.inventory as i (i.id)}
            <li class="rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
              <div class="flex flex-wrap items-baseline justify-between gap-2">
                <span class="font-semibold text-neutral-100">
                  <span class="mr-1 text-xs px-1.5 py-0.5 rounded bg-neutral-700 text-neutral-300">{i.location}</span>
                  {i.name}
                </span>
    ...
    {#if mode === 'edit'}
      <div class="mt-3 grid gap-2 grid-cols-1 sm:grid-cols-3">
        <button
          type="button"
